...
/***
 * Call back function used to set the AON Timer with the SNTP response
 * @param sec
 */
void WifiHelper::setTimeSec(uint32_t sec, uint32_t us){

	struct timespec ts;
	uint64_t microsec = sec + (60 * WifiHelper::sntpTimezoneMinutesOffset);
	microsec = microsec * 1000000 + us;

	us_to_timespec(microsec, &ts);
...
void sntpSetTimeUSec(uint32_t sec,  uint32_t us){
	WifiHelper::setTimeSec(sec, us);
}

uint8_t WifiHelper::sntpServerCount = 0;
int32_t WifiHelper::sntpTimezoneMinutesOffset = 0;

char * WifiHelper::pBuffer = NULL;
size_t WifiHelper::xBufferSize = 0;


size_t WifiHelper::ssidScan(char * buffer,  size_t bufferLen){
	memset(buffer, 0, bufferLen);
	pBuffer = buffer;
	xBufferSize = bufferLen;
	cyw43_arch_enable_sta_mode();
	cyw43_wifi_scan_options_t scan_options = {0};
	int err = cyw43_wifi_scan(&cyw43_state, &scan_options, NULL, WifiHelper::scanResult);
...
int WifiHelper::scanResult(void *env, const cyw43_ev_scan_result_t *result) {
    if (result) {
    	/*
        ...
        if (pBuffer != NULL){
        	int next = 0;
        	int l = strlen((const char*)result->ssid);
        	if (l == 0){
        		return 0;
        	}
        	//printf("For %s(%d)\n", result->ssid, l);
        	for (;;){
        		//printf("Test pos %d\n", next);
				if (pBuffer[next] == 0){
					if (l + next < xBufferSize){
						strcpy(&pBuffer[next],  (const char*)result->ssid);
						next += l + 1;
						//printf("Copying in %d char, next %d\n", l,  next);
					}
					break;
				} else {
					if (strcmp(&pBuffer[next] ,  (const char*)result->ssid) == 0){
						//printf("Found already existed in list\n");
						break;
					} else {
						next += strlen(&pBuffer[next])+ 1;
					}
				}
        	}
        }
